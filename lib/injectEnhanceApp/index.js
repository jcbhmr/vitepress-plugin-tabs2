...
export default function injectEnhanceApp(config, enhanceAppFile) {
  const themeWrapperFilePath = fileURLToPath(
    import.meta.resolve("./themeWrapper.ts"),
  );
  const enhanceAppFilePath =
    enhanceAppFile instanceof URL
      ? fileURLToPath(enhanceAppFile)
      : enhanceAppFile.toString();
  ...
  if (!themeIndexAlias) {
    ...
    themeIndexAlias = {
      find: "@theme/index",
      replacement: join(themeAlias.replacement, "index"),
    };
    config.resolve.alias.unshift(themeIndexAlias);
  }
  config.resolve.alias.push({
    find: "~vitepress-plugin-tabs2/user-theme",
    replacement: themeIndexAlias.replacement,
  });
  themeIndexAlias.replacement = themeWrapperFilePath;
  config.resolve.alias.push({
    find: "~vitepress-plugin-tabs2/enhance-app-theme",
    replacement: enhanceAppFilePath,
  });
}
